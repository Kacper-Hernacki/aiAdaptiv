...
function weighted(table: { c: string; w: number }[], r: number): string {
  let acc = 0;
...
export function CosmicField() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      if (i < nCereb) {
        // Cerebrum: ellipsoid shell + ridged-noise gyri.
        const yy = 1 - ((i + 0.5) / nCereb) * 2;
        const r = Math.sqrt(Math.max(0, 1 - yy * yy));
        const th = golden * i;
        const dx = Math.cos(th) * r;
        const dy = yy;
        const dz = Math.sin(th) * r;
        let ax = 1.18 * (dx > 0 ? 1.0 : 0.92); // frontal bulge (+x = right)
        let ay = 0.82 * (dy < 0 ? 0.82 : 1.0); // flatter underside
        const az = 0.82;
        const ng = noise3(dx * 1.9, dy * 1.9, dz * 1.9);
        const ridge = 1 - Math.abs(ng) * 0.2; // sulci recede inward
        brain[i * 3] = dx * ax * ridge;
        brain[i * 3 + 1] = dy * ay * ridge + 0.08;
        brain[i * 3 + 2] = dz * az * ridge;
        // Only the sharp ridge crests are bright (thin amber fold lines).
        bright[i] = Math.max(0, Math.min(1, 1 - Math.abs(ng) * 1.3));
      } else if (i < nCereb + nCbl) {
        // Cerebellum: small textured ellipsoid, back-bottom (-x, -y).
        const j = i - nCereb;
        ...
        const ridge = 1 - Math.abs(ng) * 0.26;
        const s = 0.36;
        ...
        // Brainstem: tapering stalk hanging down from the centre.
        const j = i - nCereb - nCbl;
        const tt = j / Math.max(1, COUNT - nCereb - nCbl);
        const a = golden * j;
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      colors[i] = isEdge ? weighted(EDGE, rand()) : weighted(FILL, rand());
    ...
    grd.addColorStop(0, "rgba(255,184,41,0.85)");
    grd.addColorStop(0.35, "rgba(255,184,41,0.32)");
    grd.addColorStop(1, "rgba(255,184,41,0)");
    gx.fillStyle = grd;
    ...
    const render = (now: number) => {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        szA[i] = z2;
        ssA[i] = baseSize[i] * (0.4 + depthN * 1.05) * persp;
        saA[i] =
          (0.18 + depthN * 0.82) * tw * globalAlpha * (0.4 + 0.6 * introEase);
      }
